...
def iterate_through_actions(object):
    text_file = open("D:\\OGRE_NEXT\\state_frame_SSE.txt", "w")
    text = ""
    ...
    for action in bpy.data.actions:
        ...
        if action.name.endswith(('_')):
            ...
            'crouch_', 'crawl_forward_', 'crawl_backward_'
                ...
                start_frame = 1
                end_frame = 2
                text += "const __m128i char01_" + action.name[:-1] + "[1][4] = { "
            else:
                start_frame = int(action.frame_range[0])
                end_frame = int(action.frame_range[1])            
                text += "const __m128i char01_" + action.name[:-1] + "[" + str(end_frame-1) + "][4] = { "
            #print(text)
    ...
    text += "typedef const __m128i(boxes_SSE_ptr)[4];\n"               
